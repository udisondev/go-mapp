...
func MapPersonToDTO(src domain.Person) dto.Person {
	...
	targetMainAccount, mapMainAccounterr := dwivbkkfaj(src.MainAccount)
	...
	for _, it := range src.Account {
		targetAccount, mapAccounterr := dwivbkkfaj(it)
	...
	if src.Profile != nil {
		targetProfile, mapProfileerr := xvgmrhwvxk(*src.Profile)
		if mapProfileerr != nil {
			panic(fmt.Sprintf("error map 'Person.Profile': %v", mapProfileerr.Error()))
		}
		target.Profile = targetProfile
	}
	enmType, err := MapPersonTypeToDto(src.Type)
	if err != nil {
		panic(fmt.Sprintf("error map 'Person.Type': %v", err.Error()))
	}
	target.Type = enmType
...
func dwivbkkfaj(src external.Account) (external.Account, error) {
	target := external.Account{}

	targetLogin, mapLoginerr := uikrnokpnx(src.Login)
	if mapLoginerr != nil {
...
func uikrnokpnx(src user.Login) (user.Login, error) {
	target := user.Login{}
...
func xvgmrhwvxk(src domain.Profile) (dto.Profile, error) {
	target := dto.Profile{}
